...
def detail(request, movie_id):
    ...
    movies = get_object_or_404(Movie, _id=ObjectId(movie_id))
...
def recommend(request):
    ...
    user = pd.DataFrame(list(Myrating.objects.filter(user=request.user).values())).drop(['user_id', '_id'], axis=1)
